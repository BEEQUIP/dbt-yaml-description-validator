...
YAML_DESCRIPTION_INDICATORS = {'|', '|-', '|+', '>', '>-', '>+'}


def iter_schema_files(files: list[str]) -> list[Path]:
    """
    Makes a list of paths to all files from a given list. If no list is given, the entire project is searched for schema.y*ml files.

    :param files: List with files
    :type files: list[str]
    :return: List with paths to the files
    :rtype: list[Path]
    """
    if files:
...
def _validate_schema_file(path: Path, check_fn: Callable[[str], bool], rule_name: str) -> list[str]:
    """
    Validates schema files returns a list of error strings whenever descriptions are not correct.

    :param path: Description
    :type path: Path
    :param check_fn: Description
    :type check_fn: Callable[[str], bool]
    :param rule_name: Description
    :type rule_name: str
    :return: Description
    :rtype: list[str]
    """
    ...
    for node in iter_nodes(data):
        ...
        if model_description and not validate_text(check_fn, model_description):
            errors.append(f"{path}: Model '{node_name}' failed rule '{rule_name}'")

            for col in iter_column_dicts(node):
                col_name = col.get("name")
                column_description = col.get("description")

                if column_description and not validate_text(check_fn, column_description):
                    errors.append(f"{path}: Column '{col_name}' failed rule '{rule_name}'")
    return errors
    ...
    while i < len(lines):
        ...
        if 'description:' in line:
            ...
            if match:
                ...
                if not value or value in YAML_DESCRIPTION_INDICATORS:
                    ...
                    for bl in block_lines:
                        if bl.strip():
                            stripped_lines.append(bl[block_indent:] if len(
                                bl) > block_indent else bl.lstrip())
                        else:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Validate dbt schema.y*ml descriptions.")
    ...
    if not callable(check_fn):
        print(
            f"Rule '{rule_name}' does not define a callable 'check(text)'.", file=sys.stderr)
        return EXIT_BAD_INPUT

    if args.fix and not callable(fix_fn):
        print(f"Rule '{rule_name}' does not support --fix.", file=sys.stderr)
        return EXIT_BAD_INPUT

    schema_files = iter_schema_files(args.files)
    if not schema_files:
        print("No schema.yml files where given as input or found in the repository.", file=sys.stderr)
        return EXIT_BAD_INPUT
    ...
    for path in schema_files:
        if args.fix:
            ...
            continue
        errors.extend(_validate_schema_file(path, check_fn, rule_name))
